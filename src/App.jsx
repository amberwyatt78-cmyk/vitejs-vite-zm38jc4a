...
const pct = (fieldTime, totalTime) =>
  totalTime > 0 ? Math.round((fieldTime / totalTime) * 100) : 0;
...
export default function FootyManager() {
  ...
  const [view, setView] = useState('field'); // "field" | "stats"
  ...
  const nextQuarter = () => {
    setRunning(false);
    setPlayers((prev) =>
      prev.map((p) => ({ ...p, onField: false, activeStart: null }))
    );
    setElapsed(0);
  ...
  const totalElapsedAllQuarters = (quarter - 1) * QUARTER_DURATION + elapsed;

  const timeLeft = QUARTER_DURATION - elapsed;
    ...
    <div style={styles.root}>
      ...
      <div style={styles.timerBar}>
        ...
        <div style={styles.progressOuter}>
          <div
            style={{ ...styles.progressInner, width: `${quarterProgress}%` }}
          />
        </div>
        <div style={styles.timerControls}>
          ...
          {quarter < 4 ? (
            ...
            <button
              style={{ ...styles.ctrlBtn, ...styles.ctrlBtnReset }}
              onClick={resetAll}
            >
              ↺ RESET
            </button>
              ...
              <div style={styles.sectionHeader}>
                ...
                {onField.length !== 18 && (
                  <span
                    style={{
                      ...styles.hint,
                      color: onField.length > 18 ? '#ff5f57' : '#ffd60a',
                    }}
                  >
                    {onField.length > 18
                      ? '⚠ Too many!'
                      : `${18 - onField.length} spots left`}
                  </span>
                )}
              </div>
              <div style={styles.playerGrid}>
                {onField.map((p) => (
                  <PlayerCard
                    ...
                    quarter={quarter}
                    elapsed={elapsed}
                    totalElapsed={totalElapsedAllQuarters}
...
function PlayerCard({
  ...
  const totalTime = player.fieldTime.reduce((a, b) => a + b, 0);
  const usage = pct(totalTime, totalElapsed);

  return (
    <div
      style={{ ...styles.card, ...(onField ? styles.cardOn : styles.cardOff) }}
    >
      ...
      <div style={styles.cardTimes}>
        ...
        <div style={styles.timeBlock}>
          <span style={styles.timeVal}>{formatTime(totalTime)}</span>
        ...
        <div style={styles.timeBlock}>
          <span
            style={{
              ...styles.timeVal,
              color:
                usage >= 70 ? '#00b894' : usage >= 40 ? '#ffd60a' : '#ff5f57',
            }}
          >
            {usage}%
          </span>
          <span style={styles.timeLabel}>USE</span>
        </div>
      </div>
      ...
      <button
        style={{
          ...styles.toggleBtn,
          ...(onField ? styles.toggleOff : styles.toggleOn),
        }}
        onClick={onToggle}
      >
        {onField ? '→ BENCH' : '→ FIELD'}
...
function StatsView({ players, quarter, totalElapsed }) {
  const sorted = [...players].sort(
    (a, b) =>
      b.fieldTime.reduce((x, y) => x + y, 0) -
      a.fieldTime.reduce((x, y) => x + y, 0)
  );
    ...
    <div style={styles.statsView}>
      <div style={styles.statsHeader}>
        <div style={styles.statsMeta}>
          After Q{quarter} · {formatTime(totalElapsed)} elapsed
        </div>
      </div>
      <div style={styles.statsTable}>
        <div style={styles.statsRow}>
          <div
            style={{ ...styles.statsCell, ...styles.statsCellHead, flex: 2 }}
          >
            PLAYER
          </div>
          {[1, 2, 3, 4].map((q) => (
            <div
              key={q}
              style={{ ...styles.statsCell, ...styles.statsCellHead }}
            >
              Q{q}
            </div>
          ))}
          <div style={{ ...styles.statsCell, ...styles.statsCellHead }}>
            TOTAL
          </div>
          <div style={{ ...styles.statsCell, ...styles.statsCellHead }}>%</div>
        </div>
        {sorted.map((p, i) => {
          const total = p.fieldTime.reduce((a, b) => a + b, 0);
          const usage = pct(total, totalElapsed);
          return (
            <div
              key={p.id}
              style={{
...
const styles = {
  ...
  '@media (max-width: 768px)': {
    fieldView: { gridTemplateColumns: '1fr' },
  },
  section: {},
  sectionHeader: {
    display: 'flex',
